...
#+BEGIN_SRC python
In [121]: np.mean(sgs.loc['Thorny'])
Out[121]: 89.392361111111114
#+END_SRC

~Thorny~ is one of the indexes. We get all the indexes with:

#+BEGIN_SRC python
In [122]: sgs.index
Out[122]: Index(['Thorny', 'Mac', 'Farva', 'Rabbit', 'Ursula', 'Foster'], dtype='object', name='Student')
#+END_SRC

We can iterate over the indexes and create a dictionary mapping all students to their averages.
#+BEGIN_SRC python
In [123]: {k: np.mean(sgs.loc[k]) for k in sgs.index}
Out[123]:
{'Farva': 57.517361111111114,
 'Foster': 94.704861111111114,
 'Mac': 98.142361111111114,
 'Rabbit': 63.454861111111114,
 'Thorny': 89.392361111111114,
 'Ursula': 78.454861111111114}
#+END_SRC

** Concatenate a Series to a DataFrame

Create a ~pd.Series~ object from the dictionary of students to averages.

#+BEGIN_SRC python
In [124]: avg = pd.Series({k: np.mean(sgs.loc[k]) for k in sgs.index})
...
Add this dictionary to our ~sgs~ ~pd.DataFrame~.
...
Student
Thorny      100      90      80  89.392361
Mac          88      99     111  98.142361
Farva        45      56      67  57.517361
Rabbit       59      61      67  63.454861
Ursula       73      79      83  78.454861
Foster       89      97     101  94.704861
#+END_SRC
...
In [136]: np.mean(sgs['Exam 1'])
Out[136]: 75.666666666666671
#+END_SRC

** Average of all the Columns

We can get a sequence of all the column names:
#+BEGIN_SRC python
In [140]: sgs.columns
Out[140]: Index(['Exam 1', 'Exam 2', 'Exam 3', 'avg'], dtype='object')
#+END_SRC

And use that to create a dictionary mapping items to dictionaries mapping 'Item Avg' the column average.

#+BEGIN_SRC python
In [179]: item_avgs = {col: {'Item Avg':np.mean(sgs[col])} for col in sgs.columns}
...
#+BEGIN_SRC python
In [183]: item_avgs_df = pd.DataFrame(item_avgs)
...
#+BEGIN_SRC python
In [185]: sgs.append(item_avgs_df)
Out[185]:
              Exam 1     Exam 2      Exam 3        avg
Thorny    100.000000  90.000000   80.000000  89.392361
Mac        88.000000  99.000000  111.000000  98.142361
Farva      45.000000  56.000000   67.000000  57.517361
Rabbit     59.000000  61.000000   67.000000  63.454861
Ursula     73.000000  79.000000   83.000000  78.454861
Foster     89.000000  97.000000  101.000000  94.704861
Item Avg   75.666667  80.333333   84.833333  80.277778
#+END_SRC
