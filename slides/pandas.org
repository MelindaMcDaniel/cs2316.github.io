...
Adding a column with letter grades is easier than

1. Find the letter grade for a single student.
2. Create a dictionary mapping all students to their letter grades.
3. Create a ~pd.Series~ object from this dictionary.
4. Add this dictionary to our ~sgs~ ~pd.DataFrame~.


** Finding Letter Grades
